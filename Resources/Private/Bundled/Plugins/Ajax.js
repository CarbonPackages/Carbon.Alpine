...
function Ajax(Alpine) {
    ...
    Alpine.directive("target", (el, { value, modifiers, expression }, { evaluateLater, effect }) => {
        let setTarget = (ids) => {
            ...
            let statues = modifiers.filter(
                (modifier) => ["back", "away", "error"].includes(modifier) || parseInt(modifier),
            );
...
var PendingTargets = {
    ...
    plan(plan, response) {
        plan.ids.forEach((pair) => {
            let docId = pair[0];
            let el = ["_self", "_top", "_none"].includes(docId)
        ...
        headers: Object.assign(
            {
                ...
                "X-Alpine-Target": PendingTargets.get(response)
                    .map((target) => target._ajax_id)
                    .join(" "),
            },
            settings.headers,
            control.headers,
...
// Resources/Private/Source/Plugins/Ajax.js
var Ajax_default = module_default;
export { Ajax_default as default };
