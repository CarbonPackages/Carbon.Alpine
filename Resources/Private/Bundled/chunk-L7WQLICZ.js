// Resources/Private/Source/UI/ListContext.js
...
function generateInputs(name, value, carry = []) {
    if (isObjectOrArray(value)) {
        for (let key in value) {
            carry = carry.concat(generateInputs(`${name}[${key}]`, value[key]));
        }
    } else {
...
function switchboard(Alpine, value) {
    let lookup = {};
    let current;
    ...
    let set = (newValue) => {
        ...
        if (current !== void 0) lookup[current].state = false;
        current = newValue;
