...
// Same as the official @alpinejs/focus plugin but with added configuration option for focusable() displayCheck
// https://github.com/alpinejs/alpine/discussions/4390
// https://github.com/joshuapease/alpine/pull/1
// https://github.com/joshuapease/alpine/pull/2
...
export default function (Alpine) {
    ...
    if (Alpine && typeof Alpine === "object" && Alpine.magic && Alpine.directive) {
        // This is the Alpine instance - backward compatible usage: Alpine.plugin(focus)
        return createFocusPlugin(Alpine, defaultConfig);
    } else {
        // This is a config object - new factory usage: Alpine.plugin(focus({ displayCheck: 'full' }))
        const userConfig = Alpine || {};
        return function (AlpineInstance) {
            return createFocusPlugin(AlpineInstance, { ...defaultConfig, ...userConfig });
        };
    }
}

function createFocusPlugin(Alpine, config) {
    let lastFocused;
    let currentFocused;
    ...
    Alpine.magic("focus", (el) => {
        ...
        return {
            __noscroll: false,
            __wrapAround: false,
            __displayCheck: config.displayCheck,
            within(el) {
                within = el;
                return this;
            },
